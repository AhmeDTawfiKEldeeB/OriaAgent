...
class TextToImage:
    ...
    def __init__(self, together_client: Optional[Together] = None):
        """Initialize the TextToImage class and validate environment variables."""
        self._together_client: Optional[Together] = together_client
    ...
    def together_client(self) -> Together:
        ...
        if self._together_client is None:
            api_key = getattr(settings, "TOGETHER_API_KEY", "") or os.getenv("TOGETHER_API_KEY")
            self._together_client = Together(api_key=api_key)
        return self._together_client

    @together_client.setter
    def together_client(self, client: Together) -> None:
        """Set the Together client instance."""
        self._together_client = client
...
def get_text_to_image() -> TextToImage:
    ...
    if _text_to_image_instance is None:
        _text_to_image_instance = TextToImage()
